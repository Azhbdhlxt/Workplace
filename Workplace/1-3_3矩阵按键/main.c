...
u8 gsmg_code[17]={0x3f,0x06,0x5b,0x4f,0x66,
				  ...
				  0x77,0x7c,0x39,0x5e,0x79,0x71};//共阴极0-F			//  global代表全局变量. 共阴极0-F
...
u8 key_matrix_ranks_scan(void)
{
	...
	while (KEY_MATRIX_PORT!=0xf7);//等待按键按下
...
KEY_MATRIX_PORT=0xfb;//第二列为低电平
	...
	while (KEY_MATRIX_PORT!=0xfb);//等待按键按下
...
KEY_MATRIX_PORT=0xfd;//第三列为低电平
	...
	while (KEY_MATRIX_PORT!=0xfd);//等待按键按下
...
KEY_MATRIX_PORT=0xfe;//第四列为低电平
	...
	while (KEY_MATRIX_PORT!=0xFE);//等待按键按下	
	return key_value;
}
